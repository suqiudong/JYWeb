  ...
  methods: {
    ...
    subHisBtn() {
      ...
      if (
        ...
        // console.log(data)
        axios
          .post(baseUrl + "api/his/bind_patient", data)
          .then(res => {
            console.log(res);
            this.loading = false;
          ...
          .catch(err => {
            ...
            console.log(err);
            if (window.navigator.onLine == false) {
              this.$message.error("补录失败，网络请求不稳定，请稍候再试！");
            } else {
              this.$message.error(err);
            }
          });
      ...
      if (this.name_w != "" && !phoneReg.test(this.phone_w)) {
        ...
        axios
          ...
          .catch(err => {
            ...
            console.log(err);
            if (window.navigator.onLine == false) {
              this.$message.error("网络请求不稳定，请稍候再试！");
            } else {
              this.$message.error(err);
            }
          });
    ...
    getQuery() {
      ...
      if (
        data.PHYSI_SEX_NAME == "男" ||
        data.ID_NUMBER.slice(16, 17) % 2 != 0
      ) {
