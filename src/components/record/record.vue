<template>
    <div class="record clear">
        ...
        <div class="record_main clear">
            ...
            <div class="userList">
                <el-table height="550" :data="tableData.slice((currentPage-1)*pagesize,currentPage*pagesize)"  style="width: 100%">
                    ...
                    <el-table-column prop="REGIST_FEE" :formatter="priceFormat" label="价格" width="60">
                    </el-table-column>
                    ...
                    <el-table-column label="操作">
                        <template slot-scope="scope">
                            <el-button @click="handleClick(scope.row)" type="warning"  :disabled="scope.row.EXECUTE_STATE != 0 || scope.row.RECORD_STATE != '等待中'" size="small">退号</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
        ...
        <div class="record_footer">
            <!-- <div class="block" > -->
                <el-pagination  style="margin:0 auto;margin-top:3%;line-height:40px;height:40px;width:35%;position:absolute;left:50%;top:0;transform: translate(-50%,0)" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage3" 
...
export default {
    data(){
        return{
            ...
            statusArr: [{name:"挂号",num:'1'},{name:"退号",num:'2'},{name:"挂号及退号",num:"3"}],
            statusArr_new: [{name:"挂号",num:'1'},{name:"退号",num:'2'},{name:"挂号及退号",num:"3"}],
            roomArr: [],
            docArr: [],
            hospitalArr: [],
            optionArr: [],//挂号员
    ...
    methods: {
        ...
        recordBtn(){
            ...
            // }else{
                ...
                if(this.status == "3"){
                    axios.post(baseUrl + "api/mq/register_list", JSON.stringify({
                        "keyword": this.icCard,
                        "status": "",
                            ...
                            for (var i=0;i<this.tableData.length;i++){
                                if(this.tableData[i].RECORD_STATE == 1){
                                    ...
                                    if(this.tableData[i].EXECUTE_STATE != 0){
                                        this.tableData[i].RECORD_STATE="已完成";
                                    }
        ...
        selectHospital(){
            ...
            axios.get(baseUrl + 'api/mq/register_type_new?hospital=' + this.hospitalId)
            .then(res=>{
                console.log(res);
                if(res.data.status == 200302){
        ...
        confirmBtn(){
            axios.post(baseUrl + "api/mq/register_cancel",JSON.stringify({
        ...
        // 获取挂号员列表
        getOption(){
            axios.get(baseUrl+"api/mq/depart_doctor?depart_code").then(res=>{
                console.log(res);
                if(res.data.status == 200302){
        ...
        statusFilter(val) {
            if (val) {
                ...
                if (opt['num'].toUpperCase().indexOf(val.toUpperCase()) == 0 || opt['name'].indexOf(val) == 0) {
                    tempArr.push(opt);
                }
                });
